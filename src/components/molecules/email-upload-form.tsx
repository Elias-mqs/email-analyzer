...
interface EmailUploadFormProps {
  ...
  isLoading: boolean
  disabled: boolean
  contentHasBeenReset: boolean
  setContentHasBeenReset: React.Dispatch<React.SetStateAction<boolean>>
}

export function EmailUploadForm({
  onSubmit,
  isLoading,
  disabled,
  contentHasBeenReset,
  setContentHasBeenReset,
}: EmailUploadFormProps) {
  const [emailText, setEmailText] = useState('')
  const [activeTab, setActiveTab] = useState('text')

  useEffect(() => {
    if (contentHasBeenReset) {
      setEmailText('')
      setContentHasBeenReset(false)
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="upload" className="space-y-4">
          <FileUploadZone onFileContent={handleFileContent} emailText={emailText} disabled={disabled} />
