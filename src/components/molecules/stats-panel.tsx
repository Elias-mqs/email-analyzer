...
import type { ClassificationResponse } from '@/hooks/use-email-classification'
...
export function StatsPanel({ history }: StatsPanelProps) {
  ...
  const avgConfidence = Math.round((history.reduce((acc, r) => acc + (r.confidence ?? 0), 0) / totalEmails) * 100)
  const avgProcessingTime = Math.round(history.reduce((acc, r) => acc + (r.processingTime ?? 0), 0) / totalEmails)
